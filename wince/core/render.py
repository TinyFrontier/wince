...
_KEYS = ("auth_logic", "data_write", "error_weakened", "contract_break", "runtime_effect", "blast_radius")
GUIDE = {"green": "routine review", "yellow": "slow down", "red": "high-attention change"}
...
def text(v: Verdict) -> str:
    ...
    out = [f"wince  {lvl}  {GUIDE[v.level]}",
           f"review score {round(v.risk * 100)}/100 · confidence {v.confidence:.2f} · heuristic", ""]
    if v.degraded:
    ...
    if v.reading_order:
        ...
        out += ["", "Reading order (experimental):", *(f"{r.ljust(width)}{s:.2f}" for r, s in rows)]
    return "\n".join(out)
...
def markdown(v: Verdict, note: str | None = None) -> str:
    out = [MARKER, f"**wince** {_DOT[v.level]} **{v.level.upper()}** — {GUIDE[v.level]} · review score {round(v.risk * 100)}/100 · confidence {v.confidence:.2f}", ""]
    if note:
        out += [f"_{note}_", ""]
    if v.degraded:
    ...
    if v.reasons:
        out += ["**Why**", "", *(f"- {r}" for r in v.reasons), ""]
    if v.reading_order:
        out += ["**Reading order (experimental)**", "", *(f"{i}. `{p}:{h}` — {s:.2f}" for i, (p, h, s) in enumerate(v.reading_order[:TOP], 1)), ""]
    if v.files:
