...
NOUL = (*SURFACE, "error_weakened", "contract_break", "runtime_effect")
...
LABELS = {
    ...
    "runtime_effect": "changes runtime behaviour",
}
...
def build(hunk_ids: list[str]) -> dict:
    q = {
        ...
        "runtime_effect": Noul(
            instructions="Does this change alter what the program does at runtime?",
            criteria={"true": "The file is executable code or executed configuration and the edited lines change "
                              "behaviour",
                      "false": "Documentation, a spec, a plan, a report, comments, tests only, or a formatting/rename "
                               "edit with no behaviour change"},
        ),
        "blast_radius": Score(
...
def ask(client: TypeSafeClient, f: FileChange) -> FileAnswers:
    # Only the diff: no title, ticket or description. Risk is a property of the change, not of its story.
...
def ask_all(files: list[FileChange]) -> list["FileAnswers | None"]:
