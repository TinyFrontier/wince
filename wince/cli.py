...
def evaluate(text: str, cfg: dict, offline: bool) -> scoring.Verdict:
    ...
    answers = [None] * len(sendable) if offline else questions.ask_all(sendable)
...
def check(a: argparse.Namespace) -> int:
    cfg = config.load()
    config.load_env()
    text = inputs.stdin() if a.stdin else inputs.file(a.diff) if a.diff else inputs.local(a.base or cfg["target_branch"])
    if not a.offline and not os.environ.get("TYPESAFE_API_KEY"):
        die("TYPESAFE_API_KEY is not set (use --offline for hard flags only)")
    v = evaluate(text, cfg, a.offline)
...
def ci_github(a: argparse.Namespace) -> int:
    ...
    v = evaluate(inputs.local(pr["base"]["ref"]), cfg, offline)
